...
namespace SocketManager {
    ...
    static void suspend_ap_for_seconds(uint32_t seconds){
        ...
        ESP_LOGI(TAG,"suspendendo AP por %u s (mode %d -> %d)", seconds, static_cast<int>(cur_mode), static_cast<int>(new_mode));
    ...
    static esp_err_t ws_handler(httpd_req_t* req) {
        ...
        if (strncmp(message.c_str(),"HOST:",5) == 0) {
            ...
            ESP_LOGI(TAG, "Comando FRM recebido para fd=%d", fd);
            OtaManager::handleFrmCommand(fd);
        }
        else if (message == "RST") {
            ESP_LOGI(TAG, "Comando RST recebido para fd=%d", fd);
            OtaManager::factoryAsync(fd);
        }
        else if (strncmp(message.c_str(),"OTA:",4) == 0) {
            ...
            result = "stnew";
            result += StorageManager::buildJSONSensors();
            sendToClient(fd,result.c_str());
        }
        else if (message == "LDM") {
    ...
    static void onStorageEvent(void*, esp_event_base_t, int32_t id, void* data) {
        ...
        if (evt==EventId::STO_SENSORSAVED){
            std::string json = StorageManager::buildJSONSensor(data);
            if(!json.empty()){
                std::string result = "stup"+json;
                broadcastToAllClients(result.c_str());
            }
        }
