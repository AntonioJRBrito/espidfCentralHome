...
namespace AutomationManager {
    ...
    static void executeAction(const char* device_id,uint8_t action,uint8_t inform){
        ESP_LOGW(TAG,"executeAction device=%s action=%u inform=%u",device_id,action,inform);
    ...
    static void scheduleTimerCallback(void* arg) {
        ESP_LOGW(TAG,"Timer callback executado");
    ...
    static uint32_t evaluateSchedule(CurrentTime* ct) {
        ...
        ESP_LOGW(TAG,"BestSchedule delta=%u dow=%u mod=%u",best,best_dow,best_mod);
        cJSON_ArrayForEach(entry,root){
            if (!cJSON_IsObject(entry)) continue;
            cJSON *dow_obj = cJSON_GetObjectItem(entry, "dow");
            cJSON *mod_obj = cJSON_GetObjectItem(entry, "mod");
            ...
            if(mod==best_mod){
                if(dow_mask&(1<<best_dow)){
                    schedule_now.devices_to_trigger[device_id] = action;
                    ESP_LOGI(TAG,"Device agendado: %s action=%d (dow=%d mod=%d)",device_id,action,best_dow,best_mod);
                }
            }
        }
        cJSON_Delete(root);
    ...
    static void checkSchedule_task(void* pvParameters) {
        ...
        if(seconds_until_next>0){
            if(schedule_timer){
                ...
                esp_timer_start_once(schedule_timer,(uint64_t)seconds_until_next*1000000ULL);
                ESP_LOGW(TAG,"Starting timer: seconds=%u microseconds=%llu",seconds_until_next,(uint64_t)seconds_until_next * 1000000ULL);
            }
