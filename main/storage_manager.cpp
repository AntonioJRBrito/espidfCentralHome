...
namespace StorageManager {
    ...
    std::string buildJSONSensors(){
        ESP_LOGI(TAG, "Construindo lista de dispositivos");
        cJSON *root = cJSON_CreateArray();
        std::vector<std::string> ids = StorageManager::getSensorIds();
        for (const auto &id : ids) {
            const Sensor* sensor = StorageManager::getSensor(id);
            if (!sensor) continue;
            cJSON *obj = cJSON_CreateObject();
            cJSON_AddStringToObject(obj, "id", sensor->id);
            cJSON_AddStringToObject(obj, "name", sensor->name);
            cJSON_AddNumberToObject(obj, "type", sensor->type);
            cJSON_AddNumberToObject(obj, "status", sensor->x_int);
            cJSON_AddNumberToObject(obj, "time", sensor->time);
            cJSON_AddItemToArray(root, obj);
        }
        char *jsonStr = cJSON_PrintUnformatted(root);
        std::string result = "";
    ...
    std::string buildJSONSensor(const void* data){
        if(!data){return "";}
        RequestSave requester;
        std::string requestId;
        memcpy(&requester,data,sizeof(RequestSave));
        ...
        if(requester.resquest_type==RequestTypes::REQUEST_CHAR){requestId = requester.request_char;}
        std::string result = "";
        const Sensor* sensor = StorageManager::getSensor(requestId);
        if (sensor){
            cJSON *root = cJSON_CreateArray();
            cJSON *obj = cJSON_CreateObject();
            cJSON_AddStringToObject(obj,"id",sensor->id);
    ...
    void registerSensor(Sensor* sensor) {
        ...
        if(StartedMQTT){
            ...
            EventBus::post(EventDomain::STORAGE,EventId::STO_SENSORCAD,&data_buffer,sizeof(data_buffer));
        }
