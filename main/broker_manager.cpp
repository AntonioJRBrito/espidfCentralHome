...
namespace BrokerManager {
    ...
    void removeClient(int fd) {
        if (xSemaphoreTake(s_broker_data_mutex, portMAX_DELAY) == pdTRUE) {
            ...
            for (auto const& [dev_id, client_fd] : s_device_id_to_fd) {
                if (client_fd == fd) {device_id_to_remove = dev_id;found = true;break;}
            }
            if (found) {
                s_device_id_to_fd.erase(device_id_to_remove);
                ESP_LOGI(TAG, "Cliente com Device ID '%s' (FD %d) removido.", device_id_to_remove.c_str(), fd);
            } else {
                ESP_LOGW(TAG, "Tentativa de remover cliente FD %d que não está mapeado.", fd);
            }
            xSemaphoreGive(s_broker_data_mutex);
        } else {
            ESP_LOGE(TAG, "Falha ao adquirir mutex para remover cliente. Evitando crash.");
        }
        close(fd);
    }
    ...
    void publish_message_to_device(const std::string& target_device_id, const std::string& payload) {
        if (xSemaphoreTake(s_broker_data_mutex, portMAX_DELAY) == pdTRUE) {
            ...
            if (it != s_device_id_to_fd.end()) {
                ...
                if (sent < 0) {
                    ESP_LOGE(TAG,"Falha no PUBLISH FD %d (Device '%s')(tópico '%s':%s)",target_fd,target_device_id.c_str(),topic.c_str(),strerror(errno));
                    removeClient(target_fd);
                } else {
                    ESP_LOGI(TAG,"PUBLISH enviado FD %d (Device ID '%s') no tópico '%s' (len %d)",target_fd,target_device_id.c_str(),topic.c_str(),sent);
                }
            } else {
                ESP_LOGW(TAG, "Device ID '%s' não encontrado ou não conectado. Mensagem não enviada.", target_device_id.c_str());
            }
            xSemaphoreGive(s_broker_data_mutex);
        } else {
            ESP_LOGE(TAG, "Falha ao adquirir mutex para publicar mensagem. Evitando crash.");
        }
    }
    ...
    static void send_connack(int fd, uint8_t return_code) {
        ...
        connack_packet[0] = 0x20;
        connack_packet[1] = 0x02;
        connack_packet[2] = 0x00;
        connack_packet[3] = return_code;
        int sent = send(fd, connack_packet, sizeof(connack_packet), 0);
        if (sent < 0) {
            ESP_LOGE(TAG, "Falha ao enviar CONNACK para FD %d: %s", fd, strerror(errno));
        } else {
            ESP_LOGI(TAG, "CONNACK enviado para FD %d com código %d", fd, return_code);
        }
    }
    ...
    static void send_suback(int fd, uint16_t packet_id, uint8_t return_code) {
        ...
        suback_packet[0] = 0x90;
        suback_packet[1] = 0x03;
        suback_packet[2] = (packet_id >> 8) & 0xFF;
        suback_packet[3] = packet_id & 0xFF;
        suback_packet[4] = return_code;
        int sent = send(fd, suback_packet, sizeof(suback_packet), 0);
        if (sent < 0) {
            ESP_LOGE(TAG, "Falha ao enviar SUBACK para FD %d (Packet ID %d): %s", fd, packet_id, strerror(errno));
        } else {
            ESP_LOGI(TAG, "SUBACK enviado para FD %d (Packet ID %d) com código %d", fd, packet_id, return_code);
        }
    }
    ...
    void client_handler_task(void *pvParameters) {
        ...
        bool clean_session = (connect_flags >> 1) & 0x01;
        bool will_flag = (connect_flags >> 2) & 0x01;
        uint8_t will_qos = (connect_flags >> 3) & 0x03;
        bool will_retain = (connect_flags >> 5) & 0x01;
        ...
        //client_id do mqtt será o MAC sem : ?
        client_device_id = client_id_mqtt;
        if (client_device_id.length() > MAX_DEVICE_ID_LEN) {
            ESP_LOGW(TAG, "FD %d: Device ID (Client ID) '%s' excede MAX_DEVICE_ID_LEN (%d). Desconectando.",
                    client_fd, client_device_id.c_str(), MAX_DEVICE_ID_LEN);
            send_connack(client_fd, 0x02);
            removeClient(client_fd);
            vTaskDelete(NULL);
            return;
        ...
        if (username_flag) {
            username_rx = read_mqtt_string(rx_buffer.data(), &offset, current_buffer_len);
        }
        std::string password_rx;
        if (password_flag) {
            password_rx = read_mqtt_string(rx_buffer.data(), &offset, current_buffer_len);
        }
        if (username_rx != FIXED_USERNAME || password_rx != FIXED_PASSWORD) {
            ESP_LOGW(TAG, "FD %d (ID '%s'): Autenticação falhou. User: '%s', Pass: '%s'",
                    client_fd, client_device_id.c_str(), username_rx.c_str(), password_rx.c_str());
        ...
        if (xSemaphoreTake(s_broker_data_mutex, portMAX_DELAY) == pdTRUE) {
            auto it = s_device_id_to_fd.find(client_device_id);
            if (it != s_device_id_to_fd.end()) {
                ESP_LOGW(TAG, "Device ID '%s' já conectado (FD %d). Desconectando cliente antigo (FD %d).",
                        client_device_id.c_str(), it->second, client_fd);
                close(it->second);
                s_device_id_to_fd.erase(it);
            }
            s_device_id_to_fd[client_device_id] = client_fd;
            authenticated = true;
            ESP_LOGI(TAG, "Cliente '%s' (FD %d) autenticado e registrado.", client_device_id.c_str(), client_fd);
            xSemaphoreGive(s_broker_data_mutex);
        } else {
            ESP_LOGE(TAG, "Falha ao adquirir mutex para registrar cliente '%s'. Desconectando.", client_device_id.c_str());
        ...
        while (authenticated) {
            ...
            if (bytes_read <= 0) {
                if (bytes_read == 0) {
                    ESP_LOGI(TAG, "FD %d (ID '%s'): Conexão fechada pelo cliente.", client_fd, client_device_id.c_str());
                } else if (errno == EWOULDBLOCK || errno == EAGAIN) {
                    ESP_LOGW(TAG, "FD %d (ID '%s'): Timeout de Keep Alive. Desconectando.", client_fd, client_device_id.c_str());
                } else {
                    ESP_LOGE(TAG, "FD %d (ID '%s'): Erro de recv: %s. Desconectando.", client_fd, client_device_id.c_str(), strerror(errno));
                }
                authenticated = false;
                break;
            }
            ...
            if (remaining_length == -1 || (offset + remaining_length > current_buffer_len)) {
                ESP_LOGW(TAG, "FD %d (ID '%s'): Remaining Length inválido ou buffer insuficiente para pacote tipo %d. Desconectando.",
                        client_fd, client_device_id.c_str(), packet_type);
                authenticated = false;
                break;
            }
            switch (packet_type) {
                case 0x03: {
                    ESP_LOGI(TAG, "PUBLISH recebido de FD %d (Device ID '%s').", client_fd, client_device_id.c_str());
                    ...
                    ESP_LOGI(TAG, "PUBLISH de '%s': Tópico '%s', Payload '%s'",client_device_id.c_str(), topic.c_str(), payload.c_str());
                    ...
                    if (topic.rfind(expected_topic_prefix, 0) == 0 && topic.length() > expected_topic_prefix.length()) {
                        ...
                        if (received_device_id_from_topic == client_device_id) {
                            // O payload pode ser REG, INF, SEN, SNA, ACT:<n>, BRG:<n>
                            if (payload == "REG") {
                                ESP_LOGI(TAG, "Payload REG recebido de '%s'.", client_device_id.c_str());
                                publish_message_to_device(received_device_id_from_topic,"INF");
                            } else if (payload.rfind("INF:", 0) == 0) {
                                ESP_LOGI(TAG, "Payload INF recebido de '%s'.", client_device_id.c_str());
                                ...
                                memset(&device_dto, 0, sizeof(DeviceDTO));
                                strncpy(device_dto.id, client_device_id.c_str(), sizeof(device_dto.id) - 1);
                                ...
                                ESP_LOGI(TAG, "INF_HANDLER: DTO montado para ID '%s': Nome='%s', Tipo=%u, Tempo=%u, Status=%u, X_Int=%u, X_Str='%s'",device_dto.id,device_dto.name,device_dto.type,device_dto.time,device_dto.status,device_dto.x_int,device_dto.x_str);
                                esp_err_t device_ret = StorageManager::enqueueRequest(StorageCommand::SAVE, StorageStructType::DEVICE_DATA, &device_dto, sizeof(DeviceDTO),client_fd, EventId::STO_DEVICESAVED);
                            ...
                            } else if (payload.rfind("SEN:", 0) == 0) { // Ex: SEN:temperatura=25.5
                                ESP_LOGI(TAG, "Payload SEN recebido de '%s': %s", client_device_id.c_str(), payload.c_str());
                                // EventBus::post(EventDomain::BROKER, EventId::BRK_SENSORRECEIVED, new SensorData{client_device_id, payload.substr(4)});
                            } else if (payload == "SNA") {
                                ESP_LOGI(TAG, "Payload SNA recebido de '%s'.", client_device_id.c_str());
                                // EventBus::post(EventDomain::BROKER, EventId::BRK_DEVICERECEIVED, new DeviceData{client_device_id, "SNA"});
                            } else if (payload.rfind("ACT:", 0) == 0) { // Ex: ACT:1
                                ESP_LOGI(TAG, "Payload ACT recebido de '%s': %s", client_device_id.c_str(), payload.c_str());
                                // int act_id = std::stoi(payload.substr(4)); // Converter para int
                                // EventBus::post(EventDomain::BROKER, EventId::BRK_SENSORACT, new ActuatorData{client_device_id, act_id});
                            } else if (payload.rfind("BRG:", 0) == 0) { // Ex: BRG:1
                                ESP_LOGI(TAG, "Payload BRG recebido de '%s': %s", client_device_id.c_str(), payload.c_str());
                                // int brg_id = std::stoi(payload.substr(4)); // Converter para int
                                // EventBus::post(EventDomain::BROKER, EventId::BRK_SENSORACT, new ActuatorData{client_device_id, brg_id});
                            } else {
                                ESP_LOGW(TAG, "Payload desconhecido '%s' de '%s'.", payload.c_str(), client_device_id.c_str());
                            }
                        } else {
                            ESP_LOGW(TAG, "Device ID no tópico ('%s') não corresponde ao Client ID ('%s'). Ignorando PUBLISH.",
                                    received_device_id_from_topic.c_str(), client_device_id.c_str());
                        }
                    } else {
                        ESP_LOGW(TAG, "Tópico de PUBLISH inválido ('%s') de '%s'. Esperado 'CTR/<id>'.", topic.c_str(), client_device_id.c_str());
                    }
                    break;
                }
                case 0x08: {
                    ESP_LOGI(TAG, "SUBSCRIBE recebido de FD %d (Device ID '%s').", client_fd, client_device_id.c_str());
                    ...
                    uint8_t requested_qos = rx_buffer[offset++];
                    ESP_LOGI(TAG, "SUBSCRIBE de '%s': Tópico '%s', QoS %d, Packet ID %d",client_device_id.c_str(), requested_topic.c_str(), requested_qos, packet_id);
                    // O cliente só pode subscrever em DSP/<id do device>
                    std::string expected_subscribe_topic = "DSP/" + client_device_id;
                    if (requested_topic == expected_subscribe_topic) {
                        send_suback(client_fd, packet_id, 0x00);
                        ESP_LOGI(TAG, "SUBSCRIBE para '%s' aceito para '%s'.", requested_topic.c_str(), client_device_id.c_str());
                    } else {
                        send_suback(client_fd, packet_id, 0x80);
                        ESP_LOGW(TAG, "SUBSCRIBE para tópico inválido '%s' de '%s'. Esperado '%s'.",
                                requested_topic.c_str(), client_device_id.c_str(), expected_subscribe_topic.c_str());
                    }
                    break;
                }
                case 0x0C: {
                    ESP_LOGI(TAG, "PINGREQ recebido de FD %d (Device ID '%s'). Enviando PINGRESP.", client_fd, client_device_id.c_str());
                    uint8_t pingresp_packet[] = {0xD0, 0x00};
                    send(client_fd, pingresp_packet, sizeof(pingresp_packet), 0);
                    break;
                }
                case 0x0E: {
                    ESP_LOGI(TAG, "DISCONNECT recebido de FD %d (Device ID '%s').", client_fd, client_device_id.c_str());
                    authenticated = false; // Sai do loop
                    break;
                }
                case 0x0A: {
                    ESP_LOGI(TAG, "UNSUBSCRIBE recebido de FD %d (Device ID '%s'). Ignorado no modelo 1:1.", client_fd, client_device_id.c_str());
                    // devo enviar UNSUBACK se o cliente espera uma resposta?
                    // uint16_t packet_id = (rx_buffer[offset] << 8) | rx_buffer[offset + 1];
                    // send_unsuback(client_fd, packet_id);
                    break;
                }
                default: {
                    ESP_LOGW(TAG, "Pacote MQTT desconhecido (tipo %d) de FD %d (Device ID '%s'). Desconectando.", packet_type, client_fd, client_device_id.c_str());
                    authenticated = false;
                    break;
                }
            }
    ...
    // --- Handler para BRK_PUBLISHREQUEST ---
    // Definir a estrutura de dados para o evento BRK_PUBLISHREQUEST
    // struct PublishEventData {
    //     std::string device_id;
    //     std::string payload;
    // };
    static void onPublishRequest(void*, esp_event_base_t base, int32_t id, void* data) {
        EventId evt = static_cast<EventId>(id);
        if (evt == EventId::BRK_PUBLISHREQUEST) {
            ESP_LOGI(TAG, "Evento BRK_PUBLISHREQUEST recebido.");
            // PublishEventData* pub_data = static_cast<PublishEventData*>(data);
            // if (pub_data) {
            //     publish_message_to_device(pub_data->device_id, pub_data->payload);
            //     delete pub_data;
            // } else {
            //     ESP_LOGE(TAG, "BRK_PUBLISHREQUEST com dados inválidos.");
            // }
            ESP_LOGW(TAG, "BRK_PUBLISHREQUEST recebido, mas dados não processados (estrutura de evento pendente).");
        }
    }
    ...
    esp_err_t init(){
        ...
        EventBus::regHandler(EventDomain::BROKER, &onPublishRequest, nullptr);
