...
namespace Storage {
    ...
    static void loadBinaryToPsram(const char* path, const char* k, const char* m) {
        FILE* f = fopen(path, "rb");
        if (!f) {ESP_LOGW(TAG, "Arquivo ausente: %s", path);return;}
        fseek(f, 0, SEEK_END);
        size_t sz = ftell(f);
        ...
        fclose(f);
        if(read_sz != sz){ESP_LOGE(TAG,"Erro ao ler arquivo (%s): esperado %zu, lido %zu", k, sz, read_sz);heap_caps_free(buf);return;}
        void* page_mem = heap_caps_malloc(sizeof(Page), MALLOC_CAP_SPIRAM);
        if (!page_mem) {ESP_LOGE(TAG, "Falha ao alocar PSRAM (%s)", k);heap_caps_free(buf);return;}
        Page* p_psram = reinterpret_cast<Page*>(page_mem);
        memset(p_psram, 0, sizeof(Page));
        p_psram->data = buf;
        p_psram->size = read_sz;
        p_psram->mime = m;
        StorageManager::registerPage(k, p_psram);
    ...
    esp_err_t init() {
        ...
        loadBinaryToPsram("/littlefs/img/logomarca.png","img/logomarca.png","image/png");
        loadBinaryToPsram("/littlefs/img/favicon.ico","favicon.ico","image/x-icon");
        loadAllDevices();
        loadAllSensors();
        loadAutomation();
        loadSchedule();
